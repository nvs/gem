...
// Registers the provided `callback` to fire when a unit enters the map. Note
// that registered functions are executed in registration order (FIFO).
function Unit_Event__On_Enter takes boolexpr callback returns nothing
	local integer callback_index
	local trigger the_trigger

	set the_trigger = CreateTrigger ()
	call TriggerAddCondition (the_trigger, callback)
...
// that registered functions are executed in reverse registration order (LIFO).
function Unit_Event__On_Leave takes boolexpr callback returns nothing
	local integer callback_index
	local trigger the_trigger
...
// registered functions are executed in registration order (FIFO).
function Unit_Event__On_Death takes boolexpr callback returns nothing
	local integer callback_index
	local trigger the_trigger
