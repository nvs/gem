...
local function text (input, ...)
	local output =
		String.trim_right(input):gsub('|n\n+', '|n'):gsub('\n', ' ')
...
do
	...
	local function attack (input, output)
		...
		for field, abbreviation in pairs(fields) do
			...
			fields[field] = object[key] and object[key].value
				or default[field]
		end
		...
		if fields.base > 0 then
			...
			if damage then
				table.insert(
					output,
					string.format(
						'|cfffed312Damage:|r %s physical damage',
						damage
					)
				)
			end
		...
		if fields.cooldown > 0 then
			table.insert(
				output,
				string.format(
					'|cfffed312Cooldown:|r %s',
					time(fields.cooldown)
				)
			)
		end
		...
		if fields.range > 0 then
			...
			table.insert(
				output,
				string.format('|cfffed312Range:|r %s', range)
			)
		end
	...
	local function bonuses (input, output)
		...
		for _, bonus in ipairs(input.bonuses) do
			table.insert(output, string.format('- |cff77dd77%s|r', bonus))
		end
	...
	local function recipe (input, output)
		...
		table.insert(
			output,
			string.format('|cfffed312Recipe: %s|r', input.recipe.name or '')
		)
			...
			if type(item) == 'table' then
				for index, element in ipairs(item) do
					if index == 1 then
						...
						table.insert(output, '  |cfffed312or|r ' .. element)
					end
				end
			else
...
armor.]],
	...
	endurance_aura = function (id, name_or_level, level)
		...
		if not level then
			level = type(name_or_level) == 'number' and name_or_level or 1
		end
		assert(level > 0)
...
damage.]]
		...
		return {
			name = 'Splash',
			description = text(description, full, partial),
		}
	end,
...
for level = 1, 11 do
	...
	shadow_strike.description = shadow_strike.description
		.. text([[
 Costs 5 mana.]])
...
local function paraiba_frost_nova (id)
	...
	frost_nova.description = frost_nova.description
		.. text([[
