...
function Gem_Information___Random_Information takes nothing returns nothing
	...
	set text = text + "- For same named buffs and debuffs, special rules apply. Unless stated otherwise, a buff or debuff can only be applied one time by any source. If the count limit for a buff or debuff is reached, then new applications simply overwrite the old. And should a debuff or debuff have multiple level, as is the case with Opal's Haste, the highest level will have priority.|n"
